...
$(function() { // for aside
    ...
    if($('.b-inreports__aside-submenu-link').length) {
		...
		function asideSlide() {
			$('.b-inreports__aside').on('mouseover.asideSlide', 
			function (event) {
			...
			}).on('mouseout.asideSlide', function (event) {			
					$('.b-inreports__aside').css('width', '');
					$('.b-inreports__aside-menu-wrapper').getNiceScroll().resize();
				});
		};
		...
		$(window).on('load resize', function () {
			if($(window).width() < 1200) {
				$('.b-inreports__aside').off('.asideSlide');
			} else {
				asideSlide();
			}
...
$(function () {
  ...
  */
  ...
  function formatState(state) {
    ...
    // Создаем список способов оплаты. Плагин Select2; Yandex, Visa, Master
    var $state = $('<div class="' + state.element.value.toLowerCase() + '__wrapper"><svg class="' + state.element.value.toLowerCase() + '__svg">\n <use xmlns:xlink="http://www.w3.org/2000/svg" xlink:href="images/icon/svg-sprite/symbol/svg/sprite.symbol.svg#'+ state.element.value.toLowerCase() + ' ">\n</svg>\n<span class="' + state.element.value.toLowerCase() + '__text"> ' + state.text + '</span></div>');
  ...
  */
  ...
  $(document).on('click', '.b-register-requisites__close-btn', function () {
    $.magnificPopup.close();
  });
...
// Start of .js-slide-toggle function //
  ...
  $(document).on('click', '.js-slide-toggle', function (e) {
    ...
    if (thisonetoggle === ".b-user-settings__line-topic-settings" || thisonetoggle === ".b-user-settings__line-topic-autostart"  ) {
     ...
     } else if (thisonetoggle === ".b-user-settings__button-container") {
...
////////////////////////////////////////////////////
      ...
      if ( selector == '.b-personal-form__requisites' ) {

        $(selector + ' input').each( function() {
          hideErrorMessage($(this));

          if( !$('.b-personal-form').hasClass('saved-form') && $(buttontext).html() == 'Отменить добавление реквизитов' ) {
            $(this).val('');
            }

        })

        $target.slideToggle(900, function () {
          $target.removeClass('animate');
          $(window).trigger('scroll');
          $(window).trigger('resize');

          setTimeout( changeRequisitesButtonText(), 9000);
        });

        window.changeRequisitesButtonText = function () {

          let companyName;

          if ( $('#b-personal-form__requisites-button-company').is(':checked') ) {
            companyName = $('.b-personal-form__input-company-name').val();
          } else {
            companyName = $('.b-personal-form__input-individual-name').val();
          }

          if( $('.b-personal-form').hasClass('saved-form') && companyName !== '' ) {

            if ( $(selector).is(':visible') ) {
              $(buttontext).html('Скрыть реквизиты ' + companyName)
            } else {
              $(buttontext).html('Показать реквизиты ' + companyName)
            }

          } else {

            if ( $(selector).is(':visible')  ) {
              $(buttontext).html('Отменить добавление реквизитов')
            } else {
              $(buttontext).html('Добавить реквизиты компании')
            }
      ...
      } else if ( selector == '.b-personal-form__passw' ) {
        ...
        $target.slideToggle(900, function () {
          ...
          $(window).trigger('scroll');
          $(window).trigger('resize');
          setTimeout( changePasswordButtonText(), 3000);
        });

        function changePasswordButtonText () {
          if ( $(selector).is(':visible')  ) {
            $(buttontext).html('Отменить смену пароля')
          } else {
            $(buttontext).html('Сменить пароль')
          }
        }
...
/////////////////////////////////////
  ...
  ///// Testing purposes
  $('.b-user-balance__form-docs-link a').click();
  ///////


// Form Requisites switcher
  $("input[name$='requisites-radio']").on('click', function() {
      
      $('.b-personal-form__requisites input').each( function() {
        hideErrorMessage($(this));
      })
      
      let content = $(this).data('content');
      $(".b-personal-form__requisites-content-all").hide();
      $(".b-personal-form__requisites-" + content ).show();

      changeRequisitesButtonText();
      
  });

// Form button; Delete Requisites
  $(document).on('click', '.b-delete-account__delete-btn-requisites', function () {
    $.magnificPopup.close();
    $('.b-personal-form').removeClass('saved-form');
    $('.b-personal-form__requisites input').val('');
    changeRequisitesButtonText();
    $('.b-personal-data__delete-btn-requisites').css('display', 'none');
  });
...
// Form submition
  $('form:not(.no-validate)').on('submit', function (event) {
    ...
    if (validationForm($(this))) {
      ...
      if ($(this).hasClass('b-authorization__item b-authorization__item--restore-password')) {
        event.preventDefault();
        $('.b-authorization__content').fadeOut(0);
        $('#password-sent').fadeIn();
      }
      ...
      if ($(this).hasClass('b-personal-form')) {
        event.preventDefault();

        $(this).addClass('saved-form')

        let saveButton = $('.b-personal-data__save-btn button');

        saveButton.addClass('edit').delay(800).queue(function(makeGreen){
            $(this).removeClass('edit');
            makeGreen();
        });

        saveButton.html("Данные сохранены").delay(0).queue(function(changeText){
            $(this).html("Сохранить изменения");
            changeText();
        });

        window.changeRequisitesButtonText();

        if ( $('.b-personal-form__requisites').is(':visible') ) {
          $('.b-personal-data__delete-btn-requisites').css('display', 'block');
        }
        
      }

      if ($(this).hasClass('b-personal-form-popup')) {    
        event.preventDefault();
        ...
        if ( companyName !== '' ) {
          $('.b-user-balance__form-docs-link-text').html('Показать реквизиты ' + companyName)
        } else {
          $('.b-user-balance__form-docs-link-text').html('Добавить реквизиты')
        }
  ...
  $('form.b-personal-form input, form.b-personal-form-popup input').on('focusout', function (event) {
    var formButton = $(this).find('button:not(.error):not([tabindex="-1"])');
...
// Form validation
  function validationForm(form) {
    ...
    if ( $(form).is('input') ) {
      ...
      formScope = form.find('input');
    };
    formScope.each(function () {
      var formElement = $(this);
      var elementName = $(this).attr('name');
      var elementVal = formElement.val();
      var elementCleanVal = elementVal.replace(/\D+/g, ''); 
      var newPassword = formElement.closest('form').find('[name="newPassword"]').val();
      var emailPattern = /^[A-Z0-9._%+-]+@([A-Z0-9-]+\.)+[A-Z]{2,4}$/i;
      var urlPattern = /.+?\..+/i; // авторизация
      var requisitesOpen = ( $('.b-personal-form__requisites').css('display') === 'block' );
      var requisitesOpenIndividual = ( $('.b-personal-form__requisites-individual').css('display') === 'block' );
      var requisitesOpenCompany = ( $('.b-personal-form__requisites-company').css('display') === 'block' );
      var passwordMenuOpen = ( $('.b-personal-form__passw').css('display') === 'block' );
      ...
      if (requisitesOpen) {
        ...
        if (requisitesOpenCompany) {

          if ( elementName == "company" && wordCount < 2) {
              isNotFilled = false;
              showErrorMessage(formElement, 'Введите тип юр. лица и название компании');
          } 

          if ( elementName == "inn" && charCount !== 10) {
            isNotFilled = false;
            if (elementVal == "") {
              showErrorMessage(formElement, 'Введите ИНН');
            } else {
              showErrorMessage(formElement, 'ИНН должен состоять из 10 цифр');
            }
          } 
          
          if ( elementName == "kpp" && charCount !== 9 ) {
            isNotFilled = false;
            if (elementVal == "") {
              showErrorMessage(formElement, 'Введите КПП');
            } else {
              showErrorMessage(formElement, 'КПП должен состоять из 9 цифр');
            }
          } 

          if ( elementName == "rs" && charCount !== 20 ) {
            isNotFilled = false;
            if (elementVal == "") {
              showErrorMessage(formElement, 'Введите расчетный счет');
            } else {
              showErrorMessage(formElement, 'Расчетный счет должен состоять из 20 цифр');
            }
          } 

          if ( elementName == "ks" && charCount !== 20 ) {
            isNotFilled = false;
            if (elementVal == "") {
              showErrorMessage(formElement, 'Введите корреспондентский счет');
            } else {
              showErrorMessage(formElement, 'Корреспондентский счет должен состоять из 20 цифр');
            }
          } 

          if ( elementName == "bik" && charCount !== 9 ) {
            isNotFilled = false;
            if (elementVal == "") {
              showErrorMessage(formElement, 'Введите БИК');
            } else {
              showErrorMessage(formElement, 'БИК должен состоять из 9 цифр');
            }
          } 

        }

        if (requisitesOpenIndividual) {

          if ( requisitesOpenIndividual && elementName == "individual-name" && wordCount < 3 ) {
            isNotFilled = false;
            showErrorMessage(formElement, 'Введите полное имя ИП');
          } 
            
          if ( requisitesOpenIndividual && elementName == "individual-inn" && charCount !== 12) {
            isNotFilled = false;
            if (elementVal == "") {
              showErrorMessage(formElement, 'Введите ИНН');
            } else {
              showErrorMessage(formElement, 'ИНН должен состоять из 12 цифр');
            }
          } 

          if ( elementName == "individual-rs" && charCount !== 20 ) {
            isNotFilled = false;
            if (elementVal == "") {
              showErrorMessage(formElement, 'Введите расчетный счет');
            } else {
          ...
          if ( elementName == "individual-ks" && charCount !== 20 ) {
            isNotFilled = false;
            if (elementVal == "") {
              showErrorMessage(formElement, 'Введите корреспондентский счет');
            } else {
          ...
          if ( elementName == "individual-bik" && charCount !== 9 ) {
            isNotFilled = false;
            if (elementVal == "") {
              showErrorMessage(formElement, 'Введите БИК');
            } else {
      ...
      if (passwordMenuOpen) {
        ...
        if (elementName == "old-password" && charCount < 6) {
          isNotFilled = false;
          if (elementVal == "") {
            showErrorMessage(formElement, 'Введите текущий пароль');
          } else {
            showErrorMessage(formElement, 'Пароль состоит минимум из 6 символов');
          }
        }

        if (elementName == "new-password" && charCount < 6) {
          isNotFilled = false;
          if (elementVal == "") {
            showErrorMessage(formElement, 'Введите новый пароль');
          } else {
            showErrorMessage(formElement, 'Пароль должен состоять минимум из 6 символов');
          }
        }

        if (elementName == "repeat-password") {

          isNotFilled = false;
          if (elementVal == "") {
            showErrorMessage(formElement, 'Повторите новый пароль');
          } else if (elementVal !== newPassword) {
            showErrorMessage(formElement, 'Пароли не совпадают');
          }
      ...
      if ( elementName == "phone" ) {
        ...
        var smsCheckBox = $('.b-personal-form__checkbox')

        if ( elementCleanVal.length >= 6 ) {
          smsCheckBox.removeClass('disabled');
          smsCheckBox.css('pointer-events', 'auto')
          $('.b-personal-form__checkbox input').removeAttr("disabled", "disabled");
        } else {
          smsCheckBox.addClass('disabled');
          smsCheckBox.css('pointer-events', 'none')
          $('.b-personal-form__checkbox input').prop("checked", false);
        }
  ...
  function showErrorMessage(element, errorMessage) {    
    ...
    if (element.attr('type') == 'checkbox') {
      var labelHeight =  ( parseInt( $(element).siblings('label').css('height') , 10) - 5 );
      $(element).not('.error').addClass('error').next('label').after('<div class="error-text" style="margin-top:'+ labelHeight +'px ">' + errorMessage + '</div>');
    } else {
      $(element).not('.error').removeClass('validated').addClass('error').after('<div class="error-text">' + errorMessage + '</div>');
    }
  ...
  // Input mask
  $('input[type=tel]').attr('placeholder', '+').mask("+999999?999999999", {"placeholder":""});
...
// function bannersList(windowWidth){
// 	if( windowWidth <= 767 ){
// 	  jQuery(".banners-list").addClass("owl-carousel");
// 	  jQuery(".banners-list").owlCarousel({
// 	      loop: !0,
// 	      margin: 0,
// 	      nav: !1,
// 	      items: 1,
...
// 	  });
// 	} else{
// 	  jQuery(".banners-list").owlCarousel('destroy').removeClass("owl-carousel");
// 	}
// };
